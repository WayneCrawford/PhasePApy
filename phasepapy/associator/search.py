from .tt_stations_3D import TTtable3D, Station3D


def PyramidSearching(session_tt_3D, nt, np, nr, tt):
    """
    Pyramid search scheme
    ...
    while nt_seg > 3 or np_seg > 3 or nr_seg > 3:
        ...
        for i in range(len(nr_cpt_set)):
            for j in range(len(nt_cpt_set)):
                for k in range(len(np_cpt_set)):
                    vars()['cpt_%d' % (i * len(nt_cpt_set) * len(np_cpt_set)
                                       + j * len(np_cpt_set) + k)]\
                        = ((nr_cpt_set[i][1] - 1) * (np - 1) * (nt - 1)
                           + (nt_cpt_set[j][1] - 1) * (np - 1)
                           + (np_cpt_set[k][1] - 1))

                    cpt_set.append((i, j, k, vars()['cpt_%d' %
                                   (i * len(nt_cpt_set) * len(np_cpt_set)
                                    + j * len(nt_cpt_set) + k)]))
        ...
        for i in range(len(cpt_set)):
            ...
            for j in range(len(tt)):
                ...
                if session.query(Station3D).filter(Station3D.sta == sta).all():
                    sta_id, = session.query(Station3D.id).\
                        filter(Station3D.sta == sta).first()
                    # print 'sta_id: ', sta_id
                    # now = time.time()
                    id = ((sta_id - 1) * (nt - 1) * (np - 1) * (nr - 1)
                          + cpt_set[i][3] + 1)
                    ttp3D, tts3D = session.query(TTtable3D.p, TTtable3D.s).\
                        filter(TTtable3D.id == id).first()
                    # print ttp3D,tts3D
        ...
        # determine the index of the checking point set: i is indexing nr,
        #    j is indexing nr, k is indexing np
        ...
        if np_seg > 3:
            np_1, np_2, np_1_cpt, np_2_cpt = Segments(np_seg)
            np_1_cpt = np_cpt - (np_1 - np_1_cpt)
            np_2_cpt += np_cpt - np_1
            np_cpt_set = [(np_1, np_1_cpt), (np_2, np_2_cpt)]
        else:
        ...
        if nt_seg > 3:
            nt_1, nt_2, nt_1_cpt, nt_2_cpt = Segments(nt_seg)
            nt_1_cpt = nt_cpt - (nt_1 - nt_1_cpt)
            nt_2_cpt += nt_cpt - nt_1
            nt_cpt_set = [(nt_1, nt_1_cpt), (nt_2, nt_2_cpt)]
        else:
        ...
        if nr_seg > 3:
            nr_1, nr_2, nr_1_cpt, nr_2_cpt = Segments(nr_seg)
            nr_1_cpt = nr_cpt - (nr_1 - nr_1_cpt)
            nr_2_cpt += nr_cpt - nr_1
            nr_cpt_set = [(nr_1, nr_1_cpt), (nr_2, nr_2_cpt)]
        else:
    ...
    # location = session.query(SourceGrids).\
    #     filter(SourceGrids.id == sourcegrid).first()
    # print 'location: ', location

    # if session.query(Station3D).filter(Station3D.sta==sta).all():
    #     ttp3D = session.query(TTtable3D.p).\
    #       filter(TTtable3D.sta==sta).all()[grid_index][0]
    #     tts3D = session.query(TTtable3D.s).\
    #       filter(TTtable3D.sta==sta).all()[grid_index][0]
    ...
    while True:
        ...
        for i in range(len(Z_fine)):
            for j in range(len(Y_fine)):
                for k in range(len(X_fine)):
                    vars()['cpt_%d' % (i * len(Y_fine) * len(X_fine)
                                       + j * len(Y_fine) + k)]\
                        = ((Z_fine[i] - 1) * (np - 1) * (nt - 1)
                           + (Y_fine[j] - 1) * (np - 1) + (X_fine[k] - 1))

                    cpt_set_f.append((i, j, k, vars()['cpt_%d' %
                                      (i * len(Y_fine) * len(X_fine)
                                       + j * len(Y_fine) + k)]))
        # print cpt_set_f
        ...
        # determine the index of the checking point set: i indexes nr,
        # j indexes nr, k indexes np
