...
class Pick(Base):
    ...
    def __repr__(self):
        ...
        return fmt.format(self.sta, self.chan, self.net, self.loc,
                          self.time.isoformat("T"), self.polarity,
                          self.snr, self.uncert,
                          self.t_create.isoformat("T"), self.phase,
...
class Candidate(Base):
    ...
    def __repr__(self):
        fmt = "Candidate({}, '{}', {:.2f}, {:.2f}, {}, {:d}, {}, {:d})"
        return fmt.format(self.ot.isoformat("T"), self.sta, self.d_km,
                          self.delta,
                          self.tp.isoformat("T"), self.p_modified_id,
                          self.ts.isoformat("T"), self.s_modified_id)

#   def __str__(self):
#     return "Candidate Event <%s %s %.2f %.2f %d %d>" %
#             (self.ot.isoformat("T"),self.sta,self.d_km,
#              self.delta, self.p_modified_id, self.s_modified_id)
