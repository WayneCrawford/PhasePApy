...
class Plot():
    ...
    def cluster_plot(self, assoc_ot_uncert = 3, filename=None):
        """
    ...
    def event_plot(self, assoc_id, west = -104.5, east= -94, south = 33.5, north = 37.5,
                   deltalon = 1.0, deltalat = 1.0, filename=None):
        """
...
#         stations = self.assoc_db.query(Pick.sta).filter(Pick.assoc_id==assoc_id).distinct().all()  
    ...
    def distance_singlephase(self,phase,time):
        """ 
        Return the closest value in the travel-time table to a single-phase travel time
