...
import matplotlib   # just to allow rcParams, a bit ugly
import matplotlib.pyplot as plt
# from matplotlib.pyplot import *
from sqlalchemy.orm import sessionmaker
from sqlalchemy import create_engine
from .tables3D import Pick, PickModified, Candidate, Associated
from .tt_stations_3D import Station3D
...
class Plot():
    ...
    def event_plot(self, assoc_id, west=-104.5, east=-94, south=33.5,
        ...
        """
        Plot all circles, stations, location and residuals on one map

        by calling the event number after the event been associated.
        """
        ...
        lon_eve, lat_eve = self.assoc_db.\
            query(Associated.longitude, Associated.latitude).\
            filter(Associated.id == assoc_id).first()
        radius_rainbow, s_p_rainbow, rainbow, sta_rainbow = [], [], [], []
        for match in matches:
            lon, lat = self.tt_stations_db_3D.\
                query(Station3D.longitude, Station3D.latitude).\
                filter(Station3D.sta == match.sta).first()
        ...
        for mismatch in mismatches:
            lon, lat = self.tt_stations_db_3D.\
                query(Station3D.longitude, Station3D.latitude).\
                filter(Station3D.sta == mismatch.sta).first()
            ...
            UnlocPair, = equi(m, lon, lat, mismatch.d_km, ls='--', lw=1.,
                              color=Color)
        try:
            mismatch
        except NameError:
        ...
        #       filter(Pick.assoc_id==assoc_id).distinct().all()
        stations = sta_rainbow
        lon_STA, lat_STA, sta_STA = [], [], []
        for index, sta in enumerate(stations):    # the comma is needed
            lon, lat = self.tt_stations_db_3D.\
                query(Station3D.longitude, Station3D.latitude).\
                filter(Station3D.sta == sta).first()
            lon_STA.append(lon)
            lat_STA.append(lat)
            sta_STA.append(sta)
        ...
        for xi, yi, stai in zip(x, y, sta_STA):
            plt.text(xi, yi + 1, stai, fontweight='bold', ha='center',
                     color='k')
        ...
        #   (t_create,t_update), horizontalalignment='center', fontsize=12,
        #   fontweight='bold')
        ...
        for single_phase in single_phases:
            lon, lat = self.tt_stations_db_3D.\
                query(Station3D.longitude, Station3D.latitude).\
                filter(Station3D.sta == single_phase.sta).first()
            x, y = m(lon, lat)
            pick_time = single_phase.time
            travel_time = (pick_time - ot).total_seconds()
            if single_phase.phase == 'P':
                tt, tt_uncert = singlephase(self.tt_stations_db_3D,
                                            single_phase.phase, travel_time)
                d_km = tt.d_km
                UnlocPhase, = equi(m, lon, lat, d_km, ls=':', lw=2., c='gray',
                                   zorder=4)
        ...
        for i in range(len(radius_rainbow)):
            subax.scatter(radius_rainbow[i], s_p_rainbow[i], s=50,
                          color=rainbow[i])
    ...
    def section_plot(self, assoc_id, files, seconds_ahead=5, record_length=100,
                     channel='Z'):
        ...
        for tr in ST_new:
            ...
            #                 tr.stats.delta)"
            t = np.arange(0, round(tr.stats.npts
            ...
            distance = int(gps2DistAzimuth(lat, lon, eq_lat,
                                           eq_lon)[0] / 1000.)
        ...
        #                              color='r')
        lines_picks = LineCollection(segs_picks, offsets=offsets_picks,
                                     transOffset=None, linewidths=1, color='k')
...
def shoot(lon, lat, azimuth, maxdist=None, Radius_earth=6371):
    ...
    :param azimuth:
    :param maxdist:
    :param Radius_earth: Earth's radius (km)
    ...
    if ((np.abs(np.cos(glat1)) < EPS)
            and not (np.abs(np.sin(faz)) < EPS)):
        warnings.warn("Only N-S courses are meaningful,"
                      " starting at a pole!")
